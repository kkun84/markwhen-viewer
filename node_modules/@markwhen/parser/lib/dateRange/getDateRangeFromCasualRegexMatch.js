...
import { EVENT_START_REGEX, datePartMatchIndex, from_matchIndex, to_matchIndex, from_relativeMatchIndex, from_beforeOrAfterMatchIndex, to_relativeMatchIndex, from_slashDateFullMatchIndex, to_slashDateFullMatchIndex, from_timeOnlyMatchIndex, to_timeOnlyMatchIndex, from_nowMatchIndex, to_nowMatchIndex, from_relativeEventIdMatchIndex, from_slashDateTimeMatchIndex, from_timeOnlyMeridiemHourMatchIndex, from_timeOnlyMeridiemMinuteMatchIndex, from_timeOnlyMeridiemMeridiemMatchIndex, from_timeOnly24HourHourMatchIndex, from_timeOnly24HourMinuteMatchIndex, to_relativeEventIdMatchIndex, to_slashDateTimeMatchIndex, to_timeOnlyMeridiemHourMatchIndex, to_timeOnlyMeridiemMinuteMatchIndex, to_timeOnlyMeridiemMeridiemMatchIndex, to_timeOnly24HourHourMatchIndex, to_timeOnly24HourMinuteMatchIndex, eventTextMatchIndex, from_casualMonthAndDayYearMatchIndex, from_casualMonthMonthAbbrMatchIndex, from_casualMonthMonthFullMatchIndex, from_casualMonthTimeMatchIndex, from_casualMonthYearMatchIndex, from_dayFirstCasualMonthDayMatchIndex, from_dayFirstCasualMonthMonthAbbrMatchIndex, from_dayFirstCasualMonthMonthFullMatchIndex, from_monthFirstCasualMonthDayMatchIndex, from_monthFirstCasualMonthMonthAbbrMatchIndex, from_monthFirstCasualMonthMonthFullMatchIndex, to_casualMonthAndDayYearMatchIndex, to_casualMonthMonthAbbrMatchIndex, to_casualMonthMonthFullMatchIndex, to_casualMonthYearMatchIndex, to_dayFirstCasualMonthDayMatchIndex, to_dayFirstCasualMonthMonthAbbrMatchIndex, to_dayFirstCasualMonthMonthFullMatchIndex, to_monthFirstCasualMonthDayMatchIndex, to_monthFirstCasualMonthMonthAbbrMatchIndex, to_monthFirstCasualMonthMonthFullMatchIndex, to_casualMonthTimeMatchIndex, from_relativeEventStartOrEndMatchIndex, to_relativeEventIdStartOrEndMatchIndex, to_relativeEventStartOrEndMatchIndex, from_relativeEventIdStartOrEndMatchIndex, } from "../regex.js";
import { DateRangePart, RelativeDate, RangeType, toDateRange, isEvent, } from "../Types.js";
import { getTimeFromSlashDateFrom, getTimeFromRegExpMatch, getTimeFromSlashDateTo, parseSlashDate, roundDateUp, getTimeFromCasualMonthFrom, fromCasualDate, getTimeFromCasualMonthTo, } from "./utils.js";
import { checkRecurrence } from "./checkRecurrence.js";
export function getDateRangeFromCasualRegexMatch(line, i, lengthAtIndex, context) {
    ...
    if (cached) {
        const recurrence = checkRecurrence(line, i, lengthAtIndex, eventStartLineRegexMatch, context);
        if (recurrence) {
    ...
    const relativeFromBeforeOrAfter = eventStartLineRegexMatch[from_beforeOrAfterMatchIndex];
    ...
    const fromCasual = fromCasualDate(eventStartLineRegexMatch, context, from_monthFirstCasualMonthMonthFullMatchIndex, from_monthFirstCasualMonthDayMatchIndex, from_casualMonthAndDayYearMatchIndex, from_monthFirstCasualMonthMonthAbbrMatchIndex, from_dayFirstCasualMonthDayMatchIndex, from_dayFirstCasualMonthMonthFullMatchIndex, from_dayFirstCasualMonthMonthAbbrMatchIndex, from_casualMonthYearMatchIndex, from_casualMonthMonthFullMatchIndex, from_casualMonthMonthAbbrMatchIndex, eventStartLineRegexMatch[from_casualMonthTimeMatchIndex]
        ? getTimeFromCasualMonthFrom(eventStartLineRegexMatch)
        : undefined);
    const toCasual = fromCasualDate(eventStartLineRegexMatch, context, to_monthFirstCasualMonthMonthFullMatchIndex, to_monthFirstCasualMonthDayMatchIndex, to_casualMonthAndDayYearMatchIndex, to_monthFirstCasualMonthMonthAbbrMatchIndex, to_dayFirstCasualMonthDayMatchIndex, to_dayFirstCasualMonthMonthFullMatchIndex, to_dayFirstCasualMonthMonthAbbrMatchIndex, to_casualMonthYearMatchIndex, to_casualMonthMonthFullMatchIndex, to_casualMonthMonthAbbrMatchIndex, eventStartLineRegexMatch[to_casualMonthTimeMatchIndex]
    ...
    if (relativeFromDate) {
        ...
        canCacheRange = false;
        isDependent = true;
        const relativeToEventId = eventStartLineRegexMatch[from_relativeEventIdMatchIndex];
        let dependentOn;
        if (relativeToEventId) {
            ...
            if (event && isEvent(event)) {
                ...
                const startOrEnd = eventStartLineRegexMatch[from_relativeEventIdStartOrEndMatchIndex];
                ...
                const index = line.lastIndexOf(relativeToEventId, dateRangeInText.to + 1);
        ...
        if (!dependentOn) {
            ...
            if (!priorEvent) {
                ...
                const startOrEnd = eventStartLineRegexMatch[from_relativeEventStartOrEndMatchIndex];
        ...
        if (!relativeToDate && !eventEndDate) {
            if (fromBeforeOrAfter === "before") {
                ...
                fromDateTime = RelativeDate.from(relativeFromDate, dependentOn.dt, "minus");
            }
            else {
            ...
            if (fromBeforeOrAfter === "before") {
                if (relativeToDate) {
                    ...
                    endDateTime = RelativeDate.from(relativeFromDate, dependentOn.dt, "minus");
                    fromDateTime = RelativeDate.from(relativeToDate, endDateTime, "minus");
                }
                else {
        ...
        const timeComponent = eventStartLineRegexMatch[from_slashDateTimeMatchIndex];
        ...
        const parsed = parseSlashDate(slashPart, context.header.dateFormat, context);
        if (parsed) {
            if (timeComponent) {
        ...
        canCacheRange = false;
        isDependent = true;
        const timeFrom = getTimeFromRegExpMatch(eventStartLineRegexMatch, from_timeOnlyMeridiemHourMatchIndex, from_timeOnlyMeridiemMinuteMatchIndex, from_timeOnlyMeridiemMeridiemMatchIndex, from_timeOnly24HourHourMatchIndex, from_timeOnly24HourMinuteMatchIndex);
    ...
    if (!endDateTime) {
        if (relativeToDate) {
            canCacheRange = false;
            isDependent = true;
            const relativeToEventId = eventStartLineRegexMatch[to_relativeEventIdMatchIndex];
            // TODO: Replaec relativeTo with dependentOn
            let dependentOn;
            if (relativeToEventId) {
                const event = context.getById(relativeToEventId);
                if (event && isEvent(event)) {
                    const startOrEnd = eventStartLineRegexMatch[to_relativeEventIdStartOrEndMatchIndex];
                    const { fromDateTime: relativeEventFromDateTime, toDateTime: relativeEventToDateTime, } = toDateRange(event.dateRangeIso);
                    const path = context.ids[relativeToEventId];
                    const dt = startOrEnd === "start"
            ...
            if (!dependentOn) {
                const startOrEnd = eventStartLineRegexMatch[to_relativeEventStartOrEndMatchIndex];
                if (startOrEnd) {
                    const priorEvent = context.priorEvent();
                    if (priorEvent) {
            ...
            const timeComponent = eventStartLineRegexMatch[to_slashDateTimeMatchIndex];
            ...
            if (parsed) {
                if (timeComponent) {
                    ...
                    endDateTime = DateTime.fromISO(roundDateUp({
                        dateTimeIso: endDateTime.toISO(),
                        granularity: timePart.granularity,
                    }, context), { setZone: true, zone: context.timezone });
                }
                else {
            ...
            const timeTo = getTimeFromRegExpMatch(eventStartLineRegexMatch, to_timeOnlyMeridiemHourMatchIndex, to_timeOnlyMeridiemMinuteMatchIndex, to_timeOnlyMeridiemMeridiemMatchIndex, to_timeOnly24HourHourMatchIndex, to_timeOnly24HourMinuteMatchIndex);
