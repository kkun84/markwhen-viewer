...
import { Block, Image, toDateRange } from "./Types.js";
function formatDate(date) {
    return (date.setZone("utc").toISO().replaceAll(/[-:]/g, "").split(".")[0] + "Z");
}
...
export function eventToIcal({ event, url, }) {
    const summary = escapeText(event.firstLine.restTrimmed);
    const description = escapeText(event.supplemental
        ?.map((s) => {
    ...
    const isAllDay = !dr.fromDateTime.hour &&
...
export function toICal(mw, options) {
