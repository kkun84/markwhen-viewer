...
export function propertyHexReplacer(line) {
    return line.replace(/(^|\W+)#(\w+)/g, (match, preHashWhitespace, tagName, offset, str) => {
        // replace the hash with a right paren so we can parse it
        return `${preHashWhitespace})${tagName}`;
    });
}
export function checkHeaderTags(line, i, lengthAtIndex, context) {
    const replacer = (match, preHashWhitespace, tagName, offset, str) => {
...
export function checkTags(line, i, lengthAtIndex, context) {
