...
import { EDTF_START_REGEX, EVENT_START_REGEX, BCE_START_REGEX, MARKDOWN_SECTION_REGEX, COMPLETION_REGEX, } from "../regex.js";
import { RangeType, EventDescription, Event, } from "../Types.js";
...
function updateParseMetadata(event, path, dateRange, context) {
...
function checkCompletion(dateRange, line, context) {
...
export function checkEvent(line, lines, i, lengthAtIndex, context) {
    let dateRange = getDateRangeFromEDTFRegexMatch(line, i, lengthAtIndex, context);
    if (!dateRange) {
        dateRange = getDateRangeFromCasualRegexMatch(line, i, lengthAtIndex, context);
    }
    if (!dateRange) {
        dateRange = getDateRangeFromBCEDateRegexMatch(line, i, lengthAtIndex, context);
    }
    if (!dateRange) {
        return i;
    }
    const { properties, i: from, propOrder, range: propRange, } = parseProperties(lines, lengthAtIndex, i + 1, context);
    ...
    const eventDescription = new EventDescription(eventGroup, matchedListItems, completed);
    ...
    if (tz) {
        if (typeof tz === "string" || typeof tz === "number") {
            ...
            if (!from && !to) {
                context.parseMessages.push({
                    type: "error",
                    message: "Unable to parse distinct from and to timezones. Specify 'timezone.from' and/or 'timezone.to' to set zones for the start or end of ranges, respectively.",
                    pos: [dateRange.dateRangeInText.from, dateRange.dateRangeInText.to],
                });
            }
            if (from) {
    ...
    const event = new Event(line, properties, propOrder, propRange, dateRange, eventRange, dateRange.dateRangeInText, eventDescription);
    if (event) {
