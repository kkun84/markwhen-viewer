...
export function parseProperties(lines, lengthAtIndex, i, context) {
    ...
    while (typeof line !== "undefined") {
        ...
        const isComment = checkComments(line, propertiesEndLineIndex, lengthAtIndex, context);
        if (!isComment) {
        ...
        if (!isComment && !isThreeDash) {
            ...
            if (keyMatch) {
                ...
                const leadingWhitespaceLength = keyMatch.index + keyMatch[0].indexOf(keyMatch[1]);
                propertyRanges.push({
    ...
    if (propertyLines.length) {
        range = {
            ...
            to: lengthAtIndex[lastPropertiesLineIndexWithText] +
        ...
        try {
            ...
            if (e instanceof YAML.YAMLParseError) {
                context.parseMessages.push({
                    type: "error",
                    pos: e.pos.map((index) => lengthAtIndex[i] + index),
                    message: e.message,
                });
            }
        }
    ...
    if (threeDashRanges.length === 2) {
        ...
        if (lastPropertiesLineIndexWithText >= 0 &&
            ...
            const to = lengthAtIndex[lastPropertiesLineIndexWithText] +
                lines[lastPropertiesLineIndexWithText].length;
            const foldable = {
...
export function parseHeader(lines, lengthAtIndex, context) {
    ...
    while (typeof line !== "undefined") {
        ...
        const isComment = checkComments(line, headerEndLineIndex, lengthAtIndex, context);
        if (!isComment) {
        ...
        if (!isComment && !isThreeDash) {
            ...
            if (index >= 0) {
                headerRanges.push({
                    ...
                    to: lengthAtIndex[headerEndLineIndex] +
                        index +
    ...
    try {
        ...
        if (parsedHeader.view && typeof parsedHeader.view === "string") {
            parsedHeader.view = stringEmailListToArray(parsedHeader.view);
        }
    ...
    if (threeDashRanges.length === 2) {
        ...
        if (lastHeaderLineIndexWithText >= 0 &&
