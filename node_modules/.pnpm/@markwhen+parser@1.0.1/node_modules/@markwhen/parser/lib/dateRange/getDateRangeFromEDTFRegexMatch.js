...
import { EDTF_START_REGEX, edtfDatePartMatchIndex, from_edtfDateIndex, from_edtfDateMonthPart, from_edtfDateDayPart, to_edtfIndex, to_edtfDateIndex, to_edtfDateMonthPart, to_edtfDateDayPart, from_edtfBeforeOrAfterMatchIndex, from_edtfRelativeMatchIndex, to_edtfRelativeMatchIndex, from_edtfNowMatchIndex, to_edtfNowMatchIndex, from_edtfRelativeEventIdMatchIndex, to_edtfRelativeEventIdMatchIndex, edtfEventTextMatchIndex, from_edtfDateTimePartMatchIndex, from_edtfDateTime24HourMinuteMatchIndex, from_edtfDateTimeMeridiemMeridiemMatchIndex, from_edtfDateTime24HourHourMatchIndex, from_edtfDateTimeMeridiemHourMatchIndex, to_edtfDateTimePartMatchIndex, to_edtfDateTime24HourHourMatchIndex, to_edtfDateTime24HourMinuteMatchIndex, to_edtfDateTimeMeridiemHourMatchIndex, to_edtfDateTimeMeridiemMeridiemMatchIndex, from_edtfDateTimeMeridiemMinuteMatchIndex, to_edtfDateTimeMeridiemMinuteMatchIndex, from_edtfRelativeEventIdStartOrEndMatchIndex, from_edtfRelativeEventStartOrEndMatchIndex, to_edtfRelativeEventIdStartOrEndMatchIndex, to_edtfRelativeEventStartOrEndMatchIndex, ISO_WEEK_DATE_PART, } from "../regex.js";
...
function parseISOWeekDate(raw, zone) {
    ...
    const granularity = weekdayPart ? "day" : "week";
    return { dateTime, granularity };
}
export function getDateRangeFromEDTFRegexMatch(line, i, lengthAtIndex, context) {
    ...
    const edtfFromHasTime = !!eventStartLineRegexMatch[from_edtfDateTimePartMatchIndex];
    ...
    const edtfToHasTime = !!eventStartLineRegexMatch[to_edtfDateTimePartMatchIndex];
    const relativeFromBeforeOrAfter = eventStartLineRegexMatch[from_edtfBeforeOrAfterMatchIndex];
    const relativeFromDate = eventStartLineRegexMatch[from_edtfRelativeMatchIndex];
    ...
    if (cached) {
        const recurrence = checkEdtfRecurrence(line, i, lengthAtIndex, eventStartLineRegexMatch, context);
        if (recurrence) {
    ...
    if (edtfFrom) {
        ...
        if (isoWeekFrom) {
            ...
            const time = getTimeFromRegExpMatch(eventStartLineRegexMatch, from_edtfDateTimeMeridiemHourMatchIndex, from_edtfDateTimeMeridiemMinuteMatchIndex, from_edtfDateTimeMeridiemMeridiemMatchIndex, from_edtfDateTime24HourHourMatchIndex, from_edtfDateTime24HourMinuteMatchIndex);
        ...
        canCacheRange = false;
        isDependent = true;
        const relativeToEventId = eventStartLineRegexMatch[from_edtfRelativeEventIdMatchIndex];
        let dependentOn;
        if (relativeToEventId) {
            ...
            if (event && isEvent(event)) {
                ...
                const startOrEnd = eventStartLineRegexMatch[from_edtfRelativeEventIdStartOrEndMatchIndex];
        ...
        if (!dependentOn) {
            ...
            if (!priorEvent) {
                ...
                const startOrEnd = eventStartLineRegexMatch[from_edtfRelativeEventStartOrEndMatchIndex];
    ...
    if (!endDateTime) {
        let dependentOn;
        if (relativeToDate) {
            const relativeToEventId = eventStartLineRegexMatch[to_edtfRelativeEventIdMatchIndex];
            if (relativeToEventId) {
                const event = context.getById(relativeToEventId);
                if (event && isEvent(event)) {
                    // Dependent on other event
                    ...
                    isDependent = true;
                    const path = context.ids[relativeToEventId];
                    const startOrEnd = eventStartLineRegexMatch[to_edtfRelativeEventIdStartOrEndMatchIndex];
            ...
            if (!dependentOn) {
                const startOrEnd = eventStartLineRegexMatch[to_edtfRelativeEventStartOrEndMatchIndex];
                if (startOrEnd) {
                    const priorEvent = context.priorEvent();
                    if (priorEvent) {
            ...
            if (isoWeekTo) {
                ...
                const time = getTimeFromRegExpMatch(eventStartLineRegexMatch, to_edtfDateTimeMeridiemHourMatchIndex, to_edtfDateTimeMeridiemMinuteMatchIndex, to_edtfDateTimeMeridiemMeridiemMatchIndex, to_edtfDateTime24HourHourMatchIndex, to_edtfDateTime24HourMinuteMatchIndex);
                const timeDateTime = DateTime.fromISO(time.dateTimeIso);
