import { emptyTimeline, } from "./Types.js";
...
import { ISOMap, dateRangeToString, } from "./utilities/dateRangeToString.js";
...
export function parseDateRange(dateRangeString) {
    ...
    let dateRange = getDateRangeFromEDTFRegexMatch(dateRangeString, 0, [], parsingContext);
    if (!dateRange) {
        dateRange = getDateRangeFromCasualRegexMatch(dateRangeString, 0, [], parsingContext);
    }
    if (!dateRange) {
        dateRange = getDateRangeFromBCEDateRegexMatch(dateRangeString, 0, [], parsingContext);
    }
    return dateRange;
}
export function parse(timelineString, cache, now) {
    if (cache === true) {
        cache = new Caches();
    }
    const parser = {
...
export function parseHeader(timelineString) {
    const { lines, lengthAtIndex } = linesAndLengths(timelineString);
...
export function parsePastHeader(from, context, lines, lengthAtIndex, to) {
    let i = from;
...
export function parseTimeline(lines, lengthAtIndex, cache, now) {
...
export function parseICal(ical, options) {
    let markwhenText = "";
    ...
    for (const vevent of vevents) {
        ...
        markwhenText += `${dateRangeToString({
            fromDateTime,
            toDateTime,
        }, options?.formap ?? ISOMap)}: ${event.summary}\n`;
